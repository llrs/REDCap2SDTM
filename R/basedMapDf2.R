...
basedMapDf2 <- function(form_name, field_name, SDTM.DOMAIN, SDTM.VARIABLE, SDTM.TESTCD, Codelist.Spec, CODELIST.OID){
  ...
  df <- data.frame(USUBJID, redcap_event_name, var, SDTM.TESTCD)
  ...
  names(df) <- c("USUBJID", "redcap_event_name", SDTM.VARIABLE, paste0(SDTM.DOMAIN, "TESTCD"))
  return(df)
}